...
gyro_sensor, infrared_sensor = GyroSensor(Port.S1), InfraredSensor(Port.S4)
...
#<<<<<<<<<<<<<<<<<<<<
def sit_up():
    left_motor.run_time(sitUpSpeed * -1, sitUpMS, Stop.COAST, False)
    right_motor.run_time(sitUpSpeed, sitUpMS, Stop.COAST, False)
#<<<<<<<<<<<<<<<<<<<<
